/*
  ...
  1. New Tables
    - `notifications`
      ...
      - `type` (text) - notification type (1_day, 3_hours, 1_hour, 10_minutes)
      - `read` (boolean) - whether notification has been read
      - `sent` (boolean) - whether notification has been sent
      - `created_at` (timestamptz)

  2. Security
    - Enable RLS on notifications table
    - Add policies for teachers to manage their notifications

  3. Functions
    - Add function to create lesson notifications
    - Add function to check and send notifications
*/
...
BEGIN
  ...
  VALUES
    (NEW.teacher_id, NEW.student_id, NEW.id, '1_day'),
    (NEW.teacher_id, NEW.student_id, NEW.id, '3_hours'),
    (NEW.teacher_id, NEW.student_id, NEW.id, '1_hour'),
    (NEW.teacher_id, NEW.student_id, NEW.id, '10_minutes');
